...
// interface WeeklyKeywordProps {
//   searchTerm: string;
// }

// const WeeklyKeyword: React.FC<WeeklyKeywordProps> = ({ searchTerm }) => {
//   const [data2, setData2] = useState("");
//   const [sortable, setSortable] = useState<[string, number][]>([]);
//   const [selectedKeyword, setSelectedKeyword] = useState<string | null>(null);

//   const defaultData = "오염수";

//   useEffect(() => {
//     if (searchTerm !== "") {
//       setSelectedKeyword(searchTerm);
//     }
//     Naver.newstitle(selectedKeyword || defaultData).then((res: string[]) => {
//       let data = "";
//       for (var i = 0; i < res.length; i++) {
//         data += res[i];
//       }
...
//     const sortable: [string, number][] = Object.entries(result);
...
//     try {
//       const results = await Naver.onSearch(keyword);
//       console.log("검색 결과:", results);
//     } catch (error) {
//       console.error("오류 발생:", error);
//     }
...
//   function weeklyRank() {
//     return sortable.slice(0, 10).map(([rankData, _], i) => (
//       <li
//         key={rankData}
//         className="flex justify-between cursor-pointer"
//         onClick={() => handleSearch(rankData)}
//       >
//         <div>{i + 1}</div>
//         <div>{rankData}</div>
//       </li>
